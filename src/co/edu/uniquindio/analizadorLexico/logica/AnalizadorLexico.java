...
public class AnalizadorLexico {
   ...
   private static final String [] METODO_DE_MOVER={
   "Salida.' '","Left", "Right", "Up", "Down"
  ...
  public TokensAnalixador obtenerTokens(ArrayList<String> arregloArchivo,String linea,Integer i){
    ...
    public TokensAnalixador obtenerComparacion(String linea, Integer i) {
      ...
      private TokensAnalixador obtenerMover(String linea, Integer i) {
        for (String s : METODO_DE_MOVER) {
            Integer k = 0;
            if (s.charAt(k) == linea.charAt(i)) {
                k++;
                Integer j = i + 1;
                ...
                String lexema =  linea.substring(i, j);
                if (s.equals(lexema))
                    return new TokensAnalixador(TokensAnalixador.METODO_MOVER, lexema, j, contarLineas);
            }
        }
        return null;
    }
